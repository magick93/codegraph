...
async fn candidate_ddl_table() {
    ...
    let files = gen
        .generate(&mock, "CandidateType", "recruiting", &config, &tera, generate::get_project_config())
        .await
        .unwrap();
...
async fn scaffold_middleware_supports_dual_auth() {
    ...
    let files = gen
        .generate(&mock, &config, &test_generation_order(), &tera, generate::get_project_config())
        .await
        .unwrap();
...
async fn child_handler_uses_find_by_id_scoped_for_ownership() {
    ...
    let files = gen
        .generate(&mock, "RewardType", "compensation", &config, &tera, generate::get_project_config())
        .await
        .unwrap();
...
async fn fk_field_consistent_across_ddl_and_entity() {
    ...
    let ddl_files = ddl_gen
        .generate(&engine, "CandidateType", "recruiting", &config, &tera, generate::get_project_config())
        .await
        .unwrap();
...
async fn composite_range_collapses_start_end_into_daterange() {
    ...
    let gen = generate::db::ddl::DdlGenerator::new(&output_dir);
    let files = gen
        .generate(&mock, "PositionHistoryType", "common", &config, &tera, generate::get_project_config())
        .await
        .unwrap();
...
async fn recursive_child_tables_with_full_classification() {
    ...
    let gen = generate::db::ddl::DdlGenerator::new(&output_dir);
    let files = gen
        .generate(&engine, "PersonType", "common", &config, &tera, generate::get_project_config())
        .await
        .unwrap();
...
#[tokio::test]
async fn scaffold_cargo_toml_has_domain_types_and_hooks_deps() {
    let mock = setup_mock().await;
    let config = test_domain_config();
    let tera = test_tera();
    let output_dir = std::path::PathBuf::from("/tmp/hr-graph-test-harness-scaffold-deps");
    ...
    assert!(
        cargo.content.contains("hr-domain-types"),
        "Cargo.toml should have hr-domain-types path dependency"
    );
    assert!(
        cargo.content.contains("hr-hooks-api"),
        "Cargo.toml should have hr-hooks-api path dependency"
    );
    let build_deps_pos = cargo.content.find("[build-dependencies]");
    let deps_section = if let Some(pos) = build_deps_pos {
        &cargo.content[..pos]
    } else {
        cargo.content.as_str()
    };
    assert!(
        deps_section.contains("hr-domain-types"),
        "hr-domain-types must be in [dependencies], not [build-dependencies]"
    );
    assert!(
        deps_section.contains("hr-hooks-api"),
        "hr-hooks-api must be in [dependencies], not [build-dependencies]"
    );
}

#[tokio::test]
async fn hooks_output_respects_output_dir() {
    let mock = setup_mock().await;
    let config = test_domain_config();
    let tera = test_tera();
    let output_dir = tempfile::TempDir::new().unwrap();
    ...
    for file in &report.files {
        ...
        assert!(
            !path_str.contains("codegraph-core"),
            "File written to codegraph workspace: {}",
            path_str
        );
    }
    ...
    assert!(
        !report.has_errors(),
        "Generation should have no errors: {:?}",
        report.errors
    );
}

#[test]
fn project_config_defaults_are_non_empty() {
    let cfg = generate::ProjectConfig::default();
    assert!(
        !cfg.domain_types_base.is_empty(),
        "domain_types_base should default to a non-empty path"
    );
    assert!(
        !cfg.hooks_api_base.is_empty(),
        "hooks_api_base should default to a non-empty path"
    );
}
