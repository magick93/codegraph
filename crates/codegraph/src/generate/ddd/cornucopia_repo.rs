...
fn emit_adapter_tree(tree: &EntityTree, code: &mut CodeWriter) {
    ...
    if as_json {
        ...
        wln!(code, "            worker_map.insert(wr.position_id, wr.deployed_worker);");
        wln!(code, "        }}");
        wln!(code);
    }
    wln!(
    ...
    if as_json {
        ...
        for inc in &tree.tree_include {
            wln!(code, "            if let Some(worker) = worker_map.get(&row.id) {{");
            wln!(code, "            if let Some(obj) = val.as_object_mut() {{");
            wln!(
                code,
