...
impl RustCodelistGenerator {
    ...
    pub async fn generate_all(
        ...
        if !mod_entries.is_empty() {
            ...
            let mut mod_content = String::from(&format!(
    ...
    pub async fn generate_reexport_mod(&self, db: &dyn GraphQuerier) -> Result<Vec<GeneratedFile>> {
        ...
        // If no references found (e.g. domain types live in domain-specific dirs),
        // fall back to re-exporting all available codelists.
        let cfg = crate::generate::get_project_config();
        let mut content = String::from(&format!(
            "//! Codelist enum re-exports from {}.\n//! Generated by {} . DO NOT EDIT.\n\n",
            cfg.domain_types_crate, cfg.generator_name
        ));
        if used.is_empty() {
            for name in available.iter() {
                let snake = heck::ToSnakeCase::to_snake_case(name.as_str());
