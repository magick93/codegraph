...
impl EntityGenerator for SeaOrmEntityGenerator {
    ...
    async fn generate(
        ...
        if let Some(fk_field) = crate::resolve_parent_fk_column(
            schema_title,
    ...
    match db.get_composition_tree(schema_title).await {
        Ok(tree) => {
            ...
            for col in &tree.root.columns {
                ...
                if let Some((cols, _, _, _)) = crate::db::ddl::column_info_to_ddl(col, table_name) {
                    for c in cols {
    ...
    if let Ok(comp_cols) = db.get_composite_columns(&prop.name, schema_title).await {
        for col in &comp_cols {
            ...
            out.push(EntityColumn {
                ...
                is_primary_key: false,
                is_nullable,
                pg_cast: crate::pg_cast_for_type(&col.pg_type),
                sea_orm_attr: None,
            });
        }
    }
