...
impl IfmlRouteGenerator {
    ...
    /// The `$lib/roles` helper when at least one view declares roles and the
    /// framework has the SvelteKit load convention; `None` otherwise. An
...
"#,
    ...
    fn role_guard_renders_at_top_of_load_function() {
        ...
        assert!(
            rendered.contains("if (!roles.some((r) => viewRoles.includes(r))) {"),
            "{rendered}"
        );
