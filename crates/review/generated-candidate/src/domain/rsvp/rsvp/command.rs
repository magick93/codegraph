//! CQRS command handlers for Rsvp.
...
impl RsvpCommandHandler {
    ...
    pub async fn create(&self, cmd: CreateRsvpRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, RsvpError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple Rsvp records with partial-success semantics.
    /// Each item is processed in its own transaction. Returns per-item results.
    pub async fn bulk_create(
        &self,
        items: Vec<CreateRsvpRequest>,
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "Rsvp");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateRsvpRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), RsvpError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "Rsvp");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), RsvpError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "Rsvp");
