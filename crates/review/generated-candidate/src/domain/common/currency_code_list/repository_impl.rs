//! Generated repository implementation for CurrencyCodeList.
...
impl CurrencyCodeListRepository<sea_orm::DatabaseTransaction> for CurrencyCodeListRepositoryImpl {
    ...
    async fn create(
        ...
        // Insert into common.currency_code_list (direct columns)
        let model = crate::entity::common_currency_code_list::ActiveModel {
            id: Set(id),
            code: Set(cmd.code),
    ...
    #[tracing::instrument(skip(self, db), fields(db.operation = "select", db.table = "common.currency_code_list"))]
    async fn find_by_id(
        &self,
        ...
        Ok(Some(CurrencyCodeListResponse {
            id: row.id,
            code: row.code,
    ...
    #[tracing::instrument(skip(self, tx), fields(db.operation = "update", db.table = "common.currency_code_list"))]
    async fn update(
        &self,
        tx: &DatabaseTransaction,
        id: Uuid,
        ...
        // Update common.currency_code_list — only set fields present in the update request
        let mut model = crate::entity::common_currency_code_list::ActiveModel {
            id: Set(id),
            ..Default::default()
        };
    ...
    #[tracing::instrument(skip(self, tx), fields(db.operation = "delete", db.table = "common.currency_code_list"))]
    async fn delete(
        &self,
        tx: &DatabaseTransaction,
        id: Uuid,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let model = crate::entity::common_currency_code_list::Entity::find()
            .filter(crate::entity::common_currency_code_list::Column::Id.eq(id))
        ...
        let mut active: crate::entity::common_currency_code_list::ActiveModel = model.into();
    ...
    #[tracing::instrument(skip(self, db), fields(db.operation = "select_list", db.table = "common.currency_code_list"))]
    async fn list(
        &self,
        db: &DatabaseTransaction,
        page: u64,
...
;
        ...
        for row in rows {
            results.push(CurrencyCodeListResponse {
                id: row.id,
                code: row.code,
                display_name: row.display_name,
                sort_order: row.sort_order,
