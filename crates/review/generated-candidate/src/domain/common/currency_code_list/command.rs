//! CQRS command handlers for CurrencyCodeList.
...
use super::repository::CurrencyCodeListRepository;
...
#[derive(Clone)]
pub struct CurrencyCodeListCommandHandler {

    repo: Arc<dyn CurrencyCodeListRepository<sea_orm::DatabaseTransaction>>,
...
impl CurrencyCodeListCommandHandler {
    pub fn new(
    ...
    pub async fn create(&self, cmd: CreateCurrencyCodeListRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, CommonError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple CurrencyCodeList records with partial-success semantics.
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "CurrencyCodeList");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateCurrencyCodeListRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "CurrencyCodeList");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "CurrencyCodeList");
