//! CQRS command handlers for FormattedDateTime.
...
use super::repository::FormattedDateTimeRepository;
...
#[derive(Clone)]
pub struct FormattedDateTimeCommandHandler {

    repo: Arc<dyn FormattedDateTimeRepository<sea_orm::DatabaseTransaction>>,
...
impl FormattedDateTimeCommandHandler {
    pub fn new(
    ...
    pub async fn create(&self, cmd: CreateFormattedDateTimeRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, CommonError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple FormattedDateTime records with partial-success semantics.
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "FormattedDateTime");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateFormattedDateTimeRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "FormattedDateTime");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "FormattedDateTime");
