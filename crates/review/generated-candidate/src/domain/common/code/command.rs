//! CQRS command handlers for Code.
...
#[derive(Clone)]
pub struct CodeCommandHandler {

    repo: Arc<dyn CodeRepository<sea_orm::DatabaseTransaction>>,
...
impl CodeCommandHandler {
    ...
    pub async fn create(&self, cmd: CreateCodeRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, CommonError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple Code records with partial-success semantics.
    /// Each item is processed in its own transaction. Returns per-item results.
    pub async fn bulk_create(
        &self,
        items: Vec<CreateCodeRequest>,
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "Code");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateCodeRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "Code");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "Code");
