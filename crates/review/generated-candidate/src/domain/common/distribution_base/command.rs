//! CQRS command handlers for DistributionBase.
...
#[derive(Clone)]
pub struct DistributionBaseCommandHandler {

    repo: Arc<dyn DistributionBaseRepository<sea_orm::DatabaseTransaction>>,
...
impl DistributionBaseCommandHandler {
    pub fn new(
    ...
    pub async fn create(&self, cmd: CreateDistributionBaseRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, CommonError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple DistributionBase records with partial-success semantics.
    /// Each item is processed in its own transaction. Returns per-item results.
    pub async fn bulk_create(
        &self,
        items: Vec<CreateDistributionBaseRequest>,
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "DistributionBase");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateDistributionBaseRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "DistributionBase");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "DistributionBase");
