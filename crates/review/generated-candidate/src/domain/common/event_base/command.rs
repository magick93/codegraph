//! CQRS command handlers for EventBase.
...
#[derive(Clone)]
pub struct EventBaseCommandHandler {

    repo: Arc<dyn EventBaseRepository<sea_orm::DatabaseTransaction>>,
...
impl EventBaseCommandHandler {
    ...
    /// Bulk-create multiple EventBase records with partial-success semantics.
    /// Each item is processed in its own transaction. Returns per-item results.
    pub async fn bulk_create(
        &self,
        items: Vec<CreateEventBaseRequest>,
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "EventBase");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateEventBaseRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "EventBase");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "EventBase");
