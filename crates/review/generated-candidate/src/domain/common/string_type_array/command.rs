//! CQRS command handlers for StringTypeArray.
...
#[derive(Clone)]
pub struct StringTypeArrayCommandHandler {

    repo: Arc<dyn StringTypeArrayRepository<sea_orm::DatabaseTransaction>>,
...
impl StringTypeArrayCommandHandler {
    pub fn new(
    ...
    pub async fn create(&self, cmd: CreateStringTypeArrayRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<Uuid, CommonError> {
        self.create_single_in_tx(cmd, &source, correlation_id, api_key_id, organization_id, user_id, role).await
    }


    /// Bulk-create multiple StringTypeArray records with partial-success semantics.
    ...
    async fn create_single_in_tx(
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "created", "StringTypeArray");
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateStringTypeArrayRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "updated", "StringTypeArray");
    ...
    pub async fn delete(&self, id: Uuid, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid, user_id: Uuid, role: String) -> Result<(), CommonError> {
        ...
        tx.commit().await?;
        tracing::info!(entity_id = %id, event = "deleted", "StringTypeArray");
