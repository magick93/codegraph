...
impl DateQueryHandler {
    ...
    pub async fn find_by_id(&self, id: Uuid, api_key_id: Uuid, organization_id: Uuid) -> Result<Option<DateResponse>, Box<dyn std::error::Error>> {
    ...
    pub async fn list_filtered(&self, page: u64, page_size: u64, filters: &std::collections::HashMap<String, String>, api_key_id: Uuid, organization_id: Uuid) -> Result<(Vec<DateResponse>, u64), Box<dyn std::error::Error>> {
        let tx = self.db.begin().await?;
        set_rls_session_vars(&tx, api_key_id, organization_id).await?;
