...
impl ApplicationCommandHandler {
    ...
    pub async fn update(&self, id: Uuid, cmd: UpdateApplicationRequest, source: domain_types::SourceContext, correlation_id: Uuid, api_key_id: Uuid, organization_id: Uuid) -> Result<(), Box<dyn std::error::Error>> {
        let tx = self.db.begin().await?;
        set_rls_session_vars(&tx, api_key_id, organization_id, correlation_id).await?;
