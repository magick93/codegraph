...
impl ApplicationQueryHandler {
    ...
    pub async fn find_by_id_scoped(&self, id: Uuid, parent_id: Uuid, api_key_id: Uuid, organization_id: Uuid) -> Result<Option<ApplicationResponse>, Box<dyn std::error::Error>> {
        let tx = self.db.begin().await?;
        set_rls_session_vars(&tx, api_key_id, organization_id).await?;
