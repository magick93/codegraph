...
mod tests {
    ...
    fn test_pay_run_create_dto_deserializes() {
        let json = r#"{ }"#;
        let _result: Result<app::domain::compensation::pay_run::dto_create::CreatePayRunRequest, _> =
    ...
    fn test_pay_run_response_serializes() {
        ...
        let response = app::domain::compensation::pay_run::dto_response::PayRunResponse {
            id: uuid::Uuid::new_v4(),
            ..Default::default()
        };
    ...
    fn test_pay_run_create_body_single_deserializes() {
        ...
        let json = r#"{ }"#;
        let result: Result<app::api::compensation::pay_run_handler::CreatePayRunBody, _> =
            serde_json::from_str(json);
        if let Ok(body) = result {
            assert!(matches!(body, app::api::compensation::pay_run_handler::CreatePayRunBody::Single(_)));
        }
    }
    ...
    fn test_pay_run_create_body_bulk_deserializes() {
        ...
        let result: Result<app::api::compensation::pay_run_handler::CreatePayRunBody, _> =
            serde_json::from_str(json);
        if let Ok(body) = result {
            assert!(matches!(body, app::api::compensation::pay_run_handler::CreatePayRunBody::Bulk(ref items) if items.len() == 2));
        }
    }
    ...
    fn test_pay_run_create_body_empty_array_deserializes() {
        ...
        let json = r#"[]"#;
        let body: app::api::compensation::pay_run_handler::CreatePayRunBody =
            serde_json::from_str(json).expect("empty array should deserialize as CreateBody::Bulk");
        assert!(matches!(body, app::api::compensation::pay_run_handler::CreatePayRunBody::Bulk(ref items) if items.is_empty()));
    }

    #[test]
    fn test_pay_run_bulk_create_response_serializes() {
        // Verify BulkCreateResponse serializes with correct JSON structure
        let response = app::api::compensation::pay_run_handler::BulkCreateResponse {
            success: vec![
                app::domain::compensation::pay_run::dto_response::PayRunResponse {
                    id: uuid::Uuid::new_v4(),
                    ..Default::default()
                },
            ],
