...
CREATE POLICY "scope_enforced_select" ON common.event_base
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('event-base', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.event_base
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('event-base', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.event_base
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('event-base', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('event-base', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.event_base
  AS RESTRICTIVE FOR DELETE TO app_user, api_key
  USING (public.enforce_api_key_scope('event-base', id::text, 'delete'));
