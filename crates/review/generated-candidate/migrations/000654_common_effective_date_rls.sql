-- RLS policies for common.effective_date
...
ALTER TABLE common.effective_date ENABLE ROW LEVEL SECURITY;
ALTER TABLE common.effective_date FORCE ROW LEVEL SECURITY;

-- Drop all existing policies (clean slate on regeneration)
DO $$ DECLARE pol RECORD; BEGIN
  FOR pol IN SELECT policyname FROM pg_policies
    WHERE schemaname = 'common' AND tablename = 'effective_date'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON common.effective_date', pol.policyname);
  END LOOP;
END $$;
...
CREATE POLICY "scope_enforced_select" ON common.effective_date
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('effective-date', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.effective_date
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('effective-date', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.effective_date
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('effective-date', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('effective-date', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.effective_date
