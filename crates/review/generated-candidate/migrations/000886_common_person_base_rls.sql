-- RLS policies for common.person_base
...
ALTER TABLE common.person_base ENABLE ROW LEVEL SECURITY;
ALTER TABLE common.person_base FORCE ROW LEVEL SECURITY;

-- Drop all existing policies (clean slate on regeneration)
DO $$ DECLARE pol RECORD; BEGIN
  FOR pol IN SELECT policyname FROM pg_policies
    WHERE schemaname = 'common' AND tablename = 'person_base'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON common.person_base', pol.policyname);
  END LOOP;
END $$;

-- Org isolation: SELECT
CREATE POLICY "org_isolation_select" ON common.person_base
  FOR SELECT
...
CREATE POLICY "org_isolation_insert" ON common.person_base
  FOR INSERT
  WITH CHECK ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid));

-- Org isolation: UPDATE
CREATE POLICY "org_isolation_update" ON common.person_base
  FOR UPDATE
  USING ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid))
  WITH CHECK ((platform_organization_id = public.get_current_org_id()
...
CREATE POLICY "org_isolation_delete" ON common.person_base
  FOR DELETE
  USING ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid));
...
CREATE POLICY "scope_enforced_select" ON common.person_base
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('person-base', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.person_base
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('person-base', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.person_base
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('person-base', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('person-base', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.person_base
  AS RESTRICTIVE FOR DELETE TO app_user, api_key
  USING (public.enforce_api_key_scope('person-base', id::text, 'delete'));
