...
CREATE POLICY "scope_enforced_select" ON common.code
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('code', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.code
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('code', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.code
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('code', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('code', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.code
  AS RESTRICTIVE FOR DELETE TO app_user, api_key
  USING (public.enforce_api_key_scope('code', id::text, 'delete'));
