...
ALTER TABLE common.date ENABLE ROW LEVEL SECURITY;
ALTER TABLE common.date FORCE ROW LEVEL SECURITY;

-- Drop all existing policies (clean slate on regeneration)
DO $$ DECLARE pol RECORD; BEGIN
  FOR pol IN SELECT policyname FROM pg_policies
    WHERE schemaname = 'common' AND tablename = 'date'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON common.date', pol.policyname);
  END LOOP;
END $$;

-- Org isolation: SELECT
CREATE POLICY "org_isolation_select" ON common.date
  FOR SELECT
...
CREATE POLICY "org_isolation_insert" ON common.date
  FOR INSERT
  WITH CHECK ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid));

-- Org isolation: UPDATE
CREATE POLICY "org_isolation_update" ON common.date
  FOR UPDATE
  USING ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid))
  WITH CHECK ((platform_organization_id = public.get_current_org_id()
...
CREATE POLICY "org_isolation_delete" ON common.date
  FOR DELETE
  USING ((platform_organization_id = public.get_current_org_id()
    OR platform_organization_id = '00000000-0000-0000-0000-000000000000'::uuid));
...
CREATE POLICY "scope_enforced_select" ON common.date
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('date', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.date
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('date', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.date
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('date', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('date', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.date
  AS RESTRICTIVE FOR DELETE TO app_user, api_key
  USING (public.enforce_api_key_scope('date', id::text, 'delete'));
