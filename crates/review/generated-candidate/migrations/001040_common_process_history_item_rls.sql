...
CREATE POLICY "scope_enforced_select" ON common.process_history_item
  AS RESTRICTIVE FOR SELECT TO app_user, api_key
  USING (public.enforce_api_key_scope('process-history-item', id::text, 'read'));

CREATE POLICY "scope_enforced_insert" ON common.process_history_item
  AS RESTRICTIVE FOR INSERT TO app_user, api_key
  WITH CHECK (public.enforce_api_key_scope('process-history-item', '*', 'create'));

CREATE POLICY "scope_enforced_update" ON common.process_history_item
  AS RESTRICTIVE FOR UPDATE TO app_user, api_key
  USING (public.enforce_api_key_scope('process-history-item', id::text, 'update'))
  WITH CHECK (public.enforce_api_key_scope('process-history-item', id::text, 'update'));

CREATE POLICY "scope_enforced_delete" ON common.process_history_item
  AS RESTRICTIVE FOR DELETE TO app_user, api_key
  USING (public.enforce_api_key_scope('process-history-item', id::text, 'delete'));
