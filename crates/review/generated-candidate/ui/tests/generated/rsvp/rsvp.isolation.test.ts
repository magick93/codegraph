...
test.describe('Rsvp Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME Rsvp in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="rsvp-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="rsvp-empty"]');
    // Either the table is visible (without ACME data) or empty state is shown
    const tableVisible = await table.isVisible().catch(() => false);
    if (tableVisible) {
      await expect(table).not.toContainText(String(data['event'] ?? ''));
    } else {
