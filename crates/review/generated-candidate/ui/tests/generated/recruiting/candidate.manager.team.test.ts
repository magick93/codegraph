...
test.describe('Candidate Manager Team', () => {
  ...
  test.beforeAll(async ({ orgContext }) => {
    ...
    try {
      const dep_1 = await createEntityAsAcme(orgContext, '/recruiting/application', { 'applied_date': '2025-01-15' });
  ...
  test('manager can view Candidate detail', async ({ managerPage }) => {
    ...
    await expect(managerPage.locator('[data-testid="candidate-field-referred_by_application_id"]')).toBeVisible();
