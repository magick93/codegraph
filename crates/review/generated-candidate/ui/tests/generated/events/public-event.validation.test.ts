...
// Validation E2E tests for PublicEvent.
...
test.describe('PublicEvent Validation', () => {
  ...
  test('submit empty form shows errors', async ({ ownerPage }) => {
    ...
    if (await ownerPage.locator('#title').count() > 0) {
      const field = ownerPage.locator('#title');
  ...
  test('invalid field types rejected', async ({ ownerPage }) => {
    ...
    await expect(ownerPage.locator('[data-testid="public_event-submit-btn"]')).toBeVisible();
