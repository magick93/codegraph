...
test.describe('Date Validation', () => {
  ...
  test('submit empty form succeeds (no required fields)', async ({ ownerPage }) => {
    ...
    await waitForHydration(ownerPage, '[data-testid="date-submit-btn"]');
    // No required fields — submitting an empty form should succeed
    await ownerPage.locator('[data-testid="date-submit-btn"]').click();
  ...
  test('invalid field types rejected', async ({ ownerPage }) => {
    ...
    await expect(ownerPage.locator('[data-testid="date-submit-btn"]')).toBeVisible();
