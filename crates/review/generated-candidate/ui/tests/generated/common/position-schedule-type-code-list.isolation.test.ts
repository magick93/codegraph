...
test.describe.serial('PositionScheduleTypeCodeList Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME PositionScheduleTypeCodeList in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="position_schedule_type_code_list-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="position_schedule_type_code_list-empty"]');
  ...
  test('HighFive owner cannot access ACME PositionScheduleTypeCodeList by direct URL', async ({ highfiveOwnerPage }) => {
