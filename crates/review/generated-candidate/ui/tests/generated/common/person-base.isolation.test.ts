...
test.describe.serial('PersonBase Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME PersonBase in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="person_base-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="person_base-empty"]');
  ...
  test('HighFive owner cannot access ACME PersonBase by direct URL', async ({ highfiveOwnerPage }) => {
