...
test.describe('Identifier CRUD', () => {
  ...
  test('create entity via form', async ({ ownerPage: page }) => {
    ...
    await page.locator('[data-testid="identifier-submit-btn"]').click();
    await expectToast(page, 'success', 'success');

    // Should redirect to detail page

    await expect(page).toHaveURL(/\/common\/identifier\/[0-9a-f-]+/);
    ...
    if (await page.locator('[data-testid="identifier-field-scheme_agency_id"]').count() > 0) {
      await expect(page.locator('[data-testid="identifier-field-scheme_agency_id"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="identifier-field-scheme_id"]').count() > 0) {
      await expect(page.locator('[data-testid="identifier-field-scheme_id"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="identifier-field-scheme_version_id"]').count() > 0) {
      await expect(page.locator('[data-testid="identifier-field-scheme_version_id"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="identifier-field-value"]').count() > 0) {
      await expect(page.locator('[data-testid="identifier-field-value"]')).toBeVisible();
    }
