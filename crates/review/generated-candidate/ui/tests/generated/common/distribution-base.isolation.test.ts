...
function testData(): Record<string, unknown> {
  return {
    'description': 'ACME Isolation Description',
    'end_date': '2025-03-10',
    'start_date': '2025-03-10',
  };
}

test.describe.serial('DistributionBase Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME DistributionBase in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="distribution_base-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="distribution_base-empty"]');
    // Either the table is visible (without ACME data) or empty state is shown
    const tableVisible = await table.isVisible().catch(() => false);
    if (tableVisible) {
      await expect(table).not.toContainText(String(data['description'] ?? ''));
    } else {
  ...
  test('HighFive owner cannot access ACME DistributionBase by direct URL', async ({ highfiveOwnerPage }) => {
