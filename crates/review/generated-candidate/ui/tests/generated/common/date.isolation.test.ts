...
test.describe.serial('Date Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME Date in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="date-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="date-empty"]');
