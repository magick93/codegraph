...
test.describe('EventBase CRUD', () => {
  ...
  test('list page loads with table', async ({ ownerPage: page }) => {
    ...
    const table = page.locator('[data-testid="event_base-table"]');
    const empty = page.locator('[data-testid="event_base-empty"]');
  ...
  test('create entity via form', async ({ ownerPage: page }) => {
    ...
    await waitForHydration(page, '[data-testid="event_base-submit-btn"]');
    const data = testData();
    ...
    await page.locator('[data-testid="event_base-submit-btn"]').click();
    await expectToast(page, 'success', 'success');

    // Should redirect to detail page

    await expect(page).toHaveURL(/\/common\/event-base\/[0-9a-f-]+/);
    ...
    if (await page.locator('[data-testid="event_base-field-capacity"]').count() > 0) {
      await expect(page.locator('[data-testid="event_base-field-capacity"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="event_base-field-title"]').count() > 0) {
      await expect(page.locator('[data-testid="event_base-field-title"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="event_base-field-birth_date"]').count() > 0) {
      await expect(page.locator('[data-testid="event_base-field-birth_date"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="event_base-field-family_name"]').count() > 0) {
      await expect(page.locator('[data-testid="event_base-field-family_name"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="event_base-field-given_name"]').count() > 0) {
      await expect(page.locator('[data-testid="event_base-field-given_name"]')).toBeVisible();
    }
  ...
  test('detail page shows all fields', async ({ ownerPage: page, orgContext }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Event Base' })).toBeVisible();
  ...
  test('delete entity', async ({ ownerPage: page, orgContext }) => {
    ...
    await waitForHydration(page, '[data-testid="event_base-delete-btn"]');
    await page.locator('[data-testid="event_base-delete-btn"]').click();
