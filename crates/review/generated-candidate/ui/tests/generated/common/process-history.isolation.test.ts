...
test.describe.serial('ProcessHistory Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME ProcessHistory in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="process_history-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="process_history-empty"]');
  ...
  test('HighFive owner cannot access ACME ProcessHistory by direct URL', async ({ highfiveOwnerPage }) => {
