...
test.describe.serial('Code Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME Code in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="code-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="code-empty"]');
  ...
  test('HighFive owner cannot access ACME Code by direct URL', async ({ highfiveOwnerPage }) => {
