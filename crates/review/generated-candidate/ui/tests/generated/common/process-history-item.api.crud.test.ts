...
// CRUD E2E tests for ProcessHistoryItem.
...
test.describe('ProcessHistoryItem CRUD', () => {
  ...
  test('create entity via form', async ({ ownerPage: page }) => {
    ...
    await page.locator('[data-testid="process_history_item-submit-btn"]').click();
    await expectToast(page, 'success', 'success');

    // Should redirect to detail page

    await expect(page).toHaveURL(/\/common\/process-history-item\/[0-9a-f-]+/);
    ...
    if (await page.locator('[data-testid="process_history_item-field-action_date"]').count() > 0) {
      await expect(page.locator('[data-testid="process_history_item-field-action_date"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="process_history_item-field-descriptions"]').count() > 0) {
      await expect(page.locator('[data-testid="process_history_item-field-descriptions"]')).toBeVisible();
    }



    if (await page.locator('[data-testid="process_history_item-field-id"]').count() > 0) {
      await expect(page.locator('[data-testid="process_history_item-field-id"]')).toBeVisible();
    }
