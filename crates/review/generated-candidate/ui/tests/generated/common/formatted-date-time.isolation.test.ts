...
test.describe.serial('FormattedDateTime Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME FormattedDateTime in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="formatted_date_time-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="formatted_date_time-empty"]');
  ...
  test('HighFive owner cannot access ACME FormattedDateTime by direct URL', async ({ highfiveOwnerPage }) => {
