...
test.describe.serial('CurrencyCodeList Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME CurrencyCodeList in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="currency_code_list-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="currency_code_list-empty"]');
  ...
  test('HighFive owner cannot access ACME CurrencyCodeList by direct URL', async ({ highfiveOwnerPage }) => {
