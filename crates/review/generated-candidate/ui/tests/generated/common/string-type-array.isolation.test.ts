...
test.describe.serial('StringTypeArray Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME StringTypeArray in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="string_type_array-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="string_type_array-empty"]');
  ...
  test('HighFive owner cannot access ACME StringTypeArray by direct URL', async ({ highfiveOwnerPage }) => {
