...
// Validation E2E tests for FormattedDateTime.
...
test.describe('FormattedDateTime Validation', () => {


  test('submit empty form succeeds (no required fields)', async ({ ownerPage }) => {
    await ownerPage.goto(NEW_URL);
    await waitForHydration(ownerPage, '[data-testid="formatted_date_time-submit-btn"]');
    // No required fields — submitting an empty form should succeed
    await ownerPage.locator('[data-testid="formatted_date_time-submit-btn"]').click();
  ...
  test('invalid field types rejected', async ({ ownerPage }) => {
    ...
    await expect(ownerPage.locator('[data-testid="formatted_date_time-submit-btn"]')).toBeVisible();
