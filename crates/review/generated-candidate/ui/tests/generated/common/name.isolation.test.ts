...
function testData(): Record<string, unknown> {
  return {
    ...
    'formatted_name': 'ACME Isolation Formatted Name',
    'given_name': 'ACME Isolation Given Name',
  };
}

test.describe.serial('Name Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME Name in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="name-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="name-empty"]');
    // Either the table is visible (without ACME data) or empty state is shown
    const tableVisible = await table.isVisible().catch(() => false);
    if (tableVisible) {
      await expect(table).not.toContainText(String(data['family_name'] ?? ''));
    } else {
  ...
  test('HighFive owner cannot access ACME Name by direct URL', async ({ highfiveOwnerPage }) => {
