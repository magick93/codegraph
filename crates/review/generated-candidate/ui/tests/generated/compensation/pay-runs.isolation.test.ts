...
const BASE_PATH = '/compensation/pay-runs';
...
function testData(): Record<string, unknown> {
  return {
    'pay_run_id': 'ACME Isolation Pay Run Id',
    'run_date': '2025-03-10',
    'total_amount': 77,
...
test.describe('PayRun Cross-Org Isolation', () => {
  ...
  test('HighFive owner cannot see ACME PayRun in list', async ({ highfiveOwnerPage }) => {
    await highfiveOwnerPage.goto(BASE_PATH);
    // HighFive owner should see the list page but ACME's data must not appear
    const table = highfiveOwnerPage.locator('[data-testid="pay_run-table"]');
    const empty = highfiveOwnerPage.locator('[data-testid="pay_run-empty"]');
    // Either the table is visible (without ACME data) or empty state is shown
    const tableVisible = await table.isVisible().catch(() => false);
    if (tableVisible) {
      await expect(table).not.toContainText(String(data['pay_run_id'] ?? ''));
    } else {
